...
\newcommand\experiment{Photometry and Spectroscopy at the IAAT 80cm Telescope} %% experiment name
\newcommand\groupno{Group 3}       %%%%% group number
\newcommand\names{Pratyush Singh,\\
                  Proshmit Dasgutpa\\}        %%%%% full names
...
\section{Introduction}\label{sec:intro} % labels allow references, particularly important for figures and tables
  In this experiment, we use the 80 cm telescope of the Institut für Astronomie und Astrophysik Tübingen (IAAT) to study photometry and spectroscopy. These two topics are fundamental to observational astronomy 
  and are used to study the properties of stars, galaxies and other astronomical bodies. In photometry, we aim to determine the brightness of approximately 90 stars in the star cluster M37 in 
  the B and V filters which will be used to determine the age and distance to the cluster. In spectroscopy, we will look at the spectrum of one of the stars of 
  the Castor system and identify the H$\alpha$ spectral line. We will also attempt to identify any other spectral lines from the obtained spectrum.
...
\begin{itemize}
  ...
  \]
\end{itemize}
\textbf{Conclusion:} A photon of $\lambda = 4686\ \mathrm{\AA}$ generates approximately \fbox{one electron-hole pair} in silicon.
...
\textbf{Conclusion:} Since the photon energy is below the band gap of silicon, \fbox{no electron-hole pair} is generated. Therefore, silicon is not suitable for detecting infrared radiation in the K band.

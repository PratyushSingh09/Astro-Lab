...
\section{Introduction}\label{sec:intro} % labels allow references, particularly important for figures and tables
    In this experiment, we observe radio emissions from the Sun and the Milky Way using the radio telescope at Sand 1, Tübingen. We aim to detect the Sun as a point source, 
    since the resolution of our telescope isn't high enough to detect it as an extended source.\\
    We also aim to observe the 21 cm Hydrogen line to derive the galactic rotation curve and to also create a radio map of the milky way.\\
